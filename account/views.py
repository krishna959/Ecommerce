...
def cart(request):
    ...
    # if request.method =='POST':
    cart = Cart.objects.filter(is_paid=False, user=request.user).first()  # Use .first() to get a single cart
    cart.save()
    
    client = razorpay.Client(auth = (settings.KEY_ID , settings.SECRET_ID))
